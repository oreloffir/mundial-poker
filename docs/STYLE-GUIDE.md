...
|---|---|---|---|
| **Inter** | *(body default)* | 400–600 | UI copy, buttons, form labels, descriptions |
...
| `14px` | *(inline style)* | Player action badge container |
...
```css
background: rgba(13, 20, 36, 0.40);   /* pending */
...
| `chip-increase` | *(short)* | — | Green flash scale | Chip count increase |
| `chip-decrease` | *(short)* | — | Red flash scale | Chip count decrease |
| `blink` | `0.7s / 1.5s` | `ease-in-out infinite` | Opacity pulse | Timer urgency, waiting dots |
| `hero-breathe` | *(long)* | `ease infinite` | Subtle scale + opacity | Landing page ambient |
...
*Questions or discrepancies → Doni via Orel.*
