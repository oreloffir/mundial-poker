...
describe('initBettingRound — blind seeding', () => {
  ...
  it('does not seed blinds on round 2+', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 2, blindInfo5)
    expect(state.pot).toBe(0)
    expect(state.currentBet).toBe(0)
  })

  it('does not seed blinds when no blindInfo provided', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1)
    expect(state.pot).toBe(0)
    expect(state.currentBet).toBe(0)
  })

  it('stores bbPlayerIndex and bigBlindAmount for BB option detection', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1, blindInfo5)
...
describe('initBettingRound — starting position', () => {
  it('starts at UTG (seat 3) when startingSeatIndex provided', async () => {
  ...
  it('finds next active seat when target seat is folded', async () => {
    const players = makePlayers(5)
    players[3] = { ...players[3]!, hasFolded: true }
    const state = await initBettingRound('test-round', players, 1, blindInfo5, 3)
    expect(state.playerStates[state.currentPlayerIndex]!.seatIndex).toBe(4)
  })

  it('starts at first active for post-flop when dealer seat given', async () => {
  ...
  it('uses first active player when no startingSeatIndex', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1)
...
describe('getAllowedActions — BB option', () => {
  it('gives BB CHECK+RAISE when no raise beyond BB', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1, blindInfo5)
  ...
  it('gives non-BB player CALL when currentBet > totalBet', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1, blindInfo5)
  ...
  it('gives SB player CALL (needs to match BB)', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 1, blindInfo5)
  ...
  it('gives CHECK to all on round 2 with no bets', async () => {
    const state = await initBettingRound('test-round', makePlayers(5), 2)
...
describe('bet timer', () => {
  it('fires at exactly BET_TIMEOUT_MS (30s) with auto-CHECK', async () => {
    ...
    await initBettingRound('timer-1', makePlayers(3), 1)
    ...
    await clearBettingState('timer-1')
    vi.useRealTimers()
  })
  ...
  it('auto-FOLDs when CHECK not in allowedActions', async () => {
    ...
    const state = await initBettingRound('timer-2', makePlayers(5), 1, blindInfo5, 3)
    ...
    await clearBettingState('timer-2')
    vi.useRealTimers()
  })
  ...
  it('cancels timer when player acts before timeout', async () => {
    ...
    await initBettingRound('timer-3', makePlayers(3), 1)
    ...
    await clearBettingState('timer-3')
    vi.useRealTimers()
  })
})
